...
namespace :visa do
  namespace :load do
    ...
    SCRIPT_DIR  = Rails.root.join('script').to_s
    ...
    def unzip_to_temp(filename)
      ...
      AWS::S3::Base.establish_connection!(
        :access_key_id     => 'AKIAJWXU5S7NCXEVW2DQ' ,
        :secret_access_key => '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'
      )

      debugger
      unless File.exists?("#{DATA_DIR}/#{filename}")
        log_info "connecting to retrieve aws: #{filename}"
        open("#{DATA_DIR}/#{filename}", 'w') do |file|
          AWS::S3::S3Object.stream(filename, 'immifax') do |chunk|
            file.write chunk
          end
        end
      end
      ...
      log_info "moving #{filename} to target.txt"
      system `cd #{TMP_DIR} && mv *.txt target.txt`
    end

    def utf8tocsv(filename = 'target.txt')
      log_info "cleaning utf-8 from file #{filename} to target.csv"
      system `iconv -f ISO-8859-1 -t UTF-8 #{TMP_DIR}/#{filename} > #{TMP_DIR}/target.csv`
    end

    def to_mongodb(tablename, filename = 'target.csv')
      log_info "loading #{filename} to #{tablename}"
      system `mongoimport --host localhost --db immigration --collection #{tablename} --type csv --file #{TMP_DIR}/#{filename}  --headerline --upsert`
    end
...
=end
    ...
    desc 'h1b data of 2009 year'
    task :h1b_2009 => :environment do
      H1b2009.delete_all
      unzip_to_temp("H1B_efile_FY09_text.zip")
      #utf8tocsv
      to_mongodb('h1b2009s','target.txt')
=begin
      each_row_of_data do | row |
